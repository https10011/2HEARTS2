...
test('migrations apply in order on a fresh database', async () => {
  ...
  const tables = await db.query<{ name: string }>(
    `SELECT name FROM sqlite_master WHERE type = 'table' AND name IN
      ('media_assets', 'notification_registry', 'profiles', 'couple_relationship', 'important_dates', 'memories', 'memory_media', 'notes', 'timeline_events', 'reminders', 'places', 'notification_center')
     ORDER BY name`,
  );
  assert.deepStrictEqual(
    tables.map((t) => t.name),
    ['couple_relationship', 'important_dates', 'media_assets', 'memories', 'memory_media', 'notes', 'notification_center', 'notification_registry', 'places', 'profiles', 'reminders', 'timeline_events'],
  );
  await db.close();
});
